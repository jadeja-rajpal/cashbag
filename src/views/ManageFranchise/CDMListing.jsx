...
import { CDM_LISTING_CONSTANTS } from "./constants";

function CDMListing(props) {
  const { getFranchiseState, redirect } = props;
  const [currentPage, setCurrentPage] = useState(1);
  useEffect(() => {
    props.getFranchise({ userRole: "cdm", page: currentPage });
    return () => {
  ...
  const handlePageChange = (e, page) => {
    setCurrentPage(page);
    props.getFranchise({ userRole: "cdm", page });
  };
  ...
  return (
    <React.Fragment>
      ...
      <CustomTable
        columns={CDM_LISTING_CONSTANTS({ redirect })}
        showHeader={true}
