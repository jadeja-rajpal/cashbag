...
import { AM_LISTING_CONSTANTS } from "./constants";

function AMListing(props) {
  const { getFranchiseState, redirect } = props;
  const [currentPage, setCurrentPage] = useState(1);
  useEffect(() => {
    props.getFranchise({ userRole: "am", page: currentPage });
    return () => {
  ...
  const handlePageChange = (e, page) => {
    setCurrentPage(page);
    props.getFranchise({ userRole: "am", page });
  };
  ...
  return (
    <React.Fragment>
      ...
      <CustomTable
        columns={AM_LISTING_CONSTANTS({ redirect })}
        showHeader={true}
