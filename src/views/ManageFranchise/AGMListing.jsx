...
import { AGM_LISTING_CONSTANTS } from "./constants";

function AGMListing(props) {
  const { getFranchiseState, redirect } = props;
  const [currentPage, setCurrentPage] = useState(1);
  useEffect(() => {
    props.getFranchise({ userRole: "agm", page: currentPage });
    return () => {
  ...
  const handlePageChange = (e, page) => {
    setCurrentPage(page);
    props.getFranchise({ userRole: "agm", page });
  };
  ...
  return (
    <React.Fragment>
      ...
      <CustomTable
        columns={AGM_LISTING_CONSTANTS({ redirect })}
        showHeader={true}
